...
export default class Avelablity extends Component {
  ...
  enterDaysHtml() {
    let dayNamesHtml = this.dayNames.map((dayName, index) => <div className="day" key={index}><h4>{dayName}</h4></div>)
    return dayNamesHtml
  }
    ...
    return (
      <div className="avelablity">
        ...
        <div className="inputDates">
          <div><input className="formDateAv" type="text" name="formDateAv" placeholder="checkIn" value={checkIn} /></div>
          <div><input className="toDateAv" type="text" name="toDateAv" placeholder="checkOut" value={checkOut} /></div>
          <span className="red" >on the second input use the shift key and choose</span>
        </div>
      </div>
    )
  }
